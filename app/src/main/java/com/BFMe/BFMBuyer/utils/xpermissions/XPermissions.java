package com.BFMe.BFMBuyer.utils.xpermissions;

import android.app.Activity;
import android.content.pm.PackageManager;
import android.support.v4.app.ActivityCompat;

import com.BFMe.BFMBuyer.utils.xpermission.util.PermissionsBuilder;
import com.BFMe.BFMBuyer.utils.xpermission.util.PermissionsHelper;
import com.BFMe.BFMBuyer.utils.xpermission.util.Tips;
...
public class XPermissions {
	...
	public static PermissionsBuilder requestPermissions(){
		...
		xPermissions.permissionsBuilder=new PermissionsBuilder(xPermissions.activity.get());
	...
	private void permissionResult(final int requestCode,
	    ...
	    for(int i=0;i<permissions.length;i++){
	    	...
	    	if(grantResults[i] == PackageManager.PERMISSION_DENIED){
	    		...
	    		boolean isShould=ActivityCompat.shouldShowRequestPermissionRationale(xPermissions.activity.get(), permission);
	    ...
	    if(shouldMap.size() == 0){
	    	...
	    	for(Entry<String, Boolean> entry : shouldMap.entrySet()){
	    		...
	    		if(false == entry.getValue()){
	    			...
	    			new Tips(xPermissions.activity.get()).setTitle("去设置权限").setMessage(PermissionsHelper.getPermissionMsg(xPermissions.activity.get(),deniedList))
						...
						public void onOk() {
							...
							PermissionsHelper.startAppSettings(xPermissions.activity.get());
	    	...
	    	//权限不够，可以选择去重试
	    	new Tips(xPermissions.activity.get()).setTitle("权限").setMessage(PermissionsHelper.getPermissionMsg(xPermissions.activity.get(),shouldShowList))
