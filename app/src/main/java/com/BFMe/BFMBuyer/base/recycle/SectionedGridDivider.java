...
public class SectionedGridDivider extends RecyclerView.ItemDecoration {
    ...
    private boolean isDraw(RecyclerView parent, View itemView, int totalCount) {
        ...
        if (totalCount > 0 && itemPosition < totalCount - 1) {//要除去footer占有的一个位置
            if (parent.getAdapter() instanceof SectionAdapter) {
                ...
                if (adapter.hasHeader()) {
                    if (itemPosition != 0 && adapter.isSectionHeaderPosition(itemPosition - 1) ) {
                        return true;
                    }
                } else {
                    if (adapter.isSectionHeaderPosition(itemPosition)) {
                        return true;
                    }
                }
